...
def showlicense():
    ...
    DOB = "18 Jul. 1988"
    pin = 590323
    address = "Kennedy Hills, Salt Lake City"
    vt = "Two, Four"
    
    label_id["text"] = str(did)
    label_name["text"] = name
    label_DOB["text"] = DOB
    label_pin["text"] = str(pin)
    label_address["text"] = address
    label_vt["text"] = vt

    
button1 = Button(root, text="Show Driving License", command=showlicense)
...
label_vehicle_no_window = canvas.create_window(335, 180, anchor=CENTER, window=label_vt)
canvas.pack()
